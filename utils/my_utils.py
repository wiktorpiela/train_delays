...
def get_trains():
    ...
    for x in range(len(all_links)):
        start_date = pd.to_datetime(pd.read_html(all_links[x])[0].iloc[1,1]+" "+pd.read_html(all_links[x])[0].iloc[1,4],dayfirst=True)
        end_date = pd.to_datetime(pd.read_html(all_links[x])[0].iloc[-1,1]+" "+pd.read_html(all_links[x])[0].iloc[-1,4],dayfirst=True)
    ...
    for x in range(len(temp_table_sorted)):
        temp_table_sorted.iloc[x,1] = temp_table_sorted.iloc[x,1] + timedelta(seconds=int(np.random.choice(np.arange(-55,60,2))))
